...
const { api } = vi.hoisted(() => ({ api: {
  ...
  addSource: vi.fn(), changeSource: vi.fn(), deleteSource: vi.fn(),
} }));
...
describe('admin feature components', () => {
  it('renders metrics and a safe attention error', () => {
    render(<><MetricCards loading={false} error="" onRefresh={vi.fn()} metrics={{date:'2026-08-12',timezone:'Europe/Moscow',description:'Служебные события.',counts:{posts_processed:4,vacancies_added:1,skipped:2,errors:1}}} /><AttentionPanel loading={false} onResolve={vi.fn()} errors={[{id:'one',component:'llm',summary:'LLM временно недоступен',details:'Безопасные детали',status:'new',count:1,first_seen_at:'2026-08-12T00:00:00Z',last_seen_at:'2026-08-12T00:00:00Z'}]} /></>);
    ...
    expect(screen.getByText('Безопасные детали')).toBeInTheDocument();
  });
