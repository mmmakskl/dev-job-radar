...
записанные вакансии, пропуски (дубликаты, не по теме, невалидные) и ошибки
обработки. В `data/admin/metrics.jsonl` не сохраняется текст поста, поэтому
...
отклоняет закрытые/invite-ссылки и дубликаты. Источник можно выключить; удалить
