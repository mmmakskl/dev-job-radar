...
У каждой проанализированной вакансии в статусе «Подходит», «Нужна проверка»
или «Сохранён» есть кнопка **«Опубликовать»** с подтверждением. Она сначала
сохраняет вакансию и затем отправляет карточку; повторы и дубли не публикуются.
При недоступном Candidate Bot, истёкшем тексте или неопределённой доставке кнопка
отключена с объяснением. Для ручного отбора используйте предпросмотр (по умолчанию)
