...
Для ночной синхронизации раз в сутки установите приложенные systemd unit и timer
...
`journalctl -u dev-job-radar-channel-sync.service`. Таймер запускается в 04:17
с добавочной случайной задержкой до 15 минут; это уменьшает регулярные
остановки live listener и одновременные обращения к Telegram после
перезагрузки сервера. Для немедленного применения изменения папки используйте
действие **«Синхронизировать»** в панели.
...
### Карточка вакансии в канале

Канал получает одну компактную detailed-карточку. В ней последовательно
показываются заголовок и компания, зарплата (если известна), формат и локация,
грейд и занятость, стек (до шести технологий), краткое описание (до 400
символов), до четырёх хештегов и ссылка на источник (`via Telegram`, `via
Threads` или ATS-источник вроде `Greenhouse`). Неизвестные поля и пустые строки
не выводятся. Заголовок вакансии без компании использует только название, а
при отсутствии названия — нейтральное `Go Developer`.

Пример карточки:

```text
🟢 Senior Go Backend Engineer — Acme
💰 5000–6500 EUR/month
🌍 Remote · Europe
🧭 Full-time
🛠 Go · PostgreSQL · Kubernetes · Kafka
Build reliable backend services for a distributed product team…
#golang #senior #remote #europe
via Telegram
```

Ранее публикация была коротким текстовым сообщением со служебными действиями
в одной клавиатуре:

```text
Senior Go Backend Engineer — Acme
Отклик: https://example.com/jobs/123
...
Кнопка «💬 Обсудить» ботом не создаётся. Чтобы Telegram показывал нативную
ветку комментариев у каждого поста канала, владелец канала один раз вручную
подключает Discussion Group:

1. Откройте **Channel settings → Discussion**.
2. Выберите существующую группу или нажмите **Create a new group** и создайте
   группу обсуждений.
3. Проверьте права администраторов и модераторов: им нужны необходимые права
   для управления каналом и группой, включая удаление сообщений и управление
   обсуждением по вашей политике модерации.
4. Опубликуйте тестовую вакансию и убедитесь, что под постом появилась кнопка
   **Comments** и открывается связанная группа.

Бот не создаёт и не модерирует Discussion Group; модерация комментариев
остаётся ответственностью владельца и назначенных модераторов канала.
...
Поиск выполняет `channels.CheckSearchPostsFloodRequest(query=...)`, затем постраничные
запросы `channels.SearchPostsRequest(query=..., allow_paid_stars=None)`
...
Порядок исходных страниц и полноту определяет Telegram; итоговый топ ранжируется локально. Закрытые каналы, группы, приватные
...
лимит означает топ подходящих уникальных постов, а не размер первой страницы Telegram.
Поиск просматривает до 10 страниц по 100 постов. Все уникальные сообщения с проверенным
публичным источником и датой в выбранном периоде проходят Mistral без keyword-префильтра.
Точные дубли пропускаются до анализа, структурные — после. Лимит анализа — 3000 вызовов
с повторами на запуск; это увеличивает потребление Mistral. Основная выдача скрывает дубли,
отклонения и ошибки, сохраняя их в фильтрах аудита. Сначала идут подходящие, затем требующие
проверки (если включены); внутри — уверенность по убыванию, затем свежесть.
Автоматическое сохранение/публикация выполняется после ранжирования только для топ-N.
Если подходящих меньше N, возвращается фактическое число; полнота ограничена выдачей Telegram.
Строгий отдельный ответ
...
Неуверенные решения и противоречия между `is_track_match` / `analysis.is_match`
при `is_vacancy=true` и роли `primary|significant` попадают в `review`; флажок в форме влияет
только на список по умолчанию, их аудит и метрики сохраняются всегда.

У каждой карточки результата есть явная ссылка на исходную вакансию в Telegram,
цветной статус и меню действий с подтверждением: сохранить, опубликовать, отклонить,
пометить дубликатом или добавить публичный канал в источники. Публикация сначала
сохраняет вакансию и затем отправляет карточку; повторы и дубли не публикуются.
Администратор может вручную сохранить или опубликовать даже карточку, отклонённую
автоматическим фильтром. Если сохранённого анализа нет, выполняется один отдельный
анализ для формирования карточки; при ошибке анализа отправка не выполняется,
а причина отображается в интерфейсе. Это не ослабляет автоматический отбор.
При недоступном Candidate Bot, истёкшем тексте или неопределённой доставке пункт
публикации отключён с объяснением. Для ручного отбора используйте предпросмотр (по умолчанию)
и включённый флажок показа неуверенных результатов. Расписание не добавлено.
...
## Общий поиск: Telegram, Premium и Threads

Раздел **«Общий поиск»** (`/search`) доступен авторизованному администратору
независимо от флагов Premium и Threads. Он объединяет три источника:

| Источник | Что ищется |
|---|---|
| Telegram · опубликованные | Локальные `title`, `company`, `summary` уже опубликованных карточек из существующей Candidate SQLite |
| Telegram Premium | Публичные Telegram-посты через существующий Premium worker, если он включён |
| Threads | Текстовые посты через официальный Meta Threads keyword search, если настроены доступ и флаг |

Локальный Telegram-поиск не перечитывает каналы или Google Sheets. Старые записи,
которые есть только в Sheets и не публиковались через Candidate Bot, в нём не
найдутся. При отсутствии Candidate cache локальная выдача будет пустой. Исходные
Premium UI, API, история, фильтры и правила расходования бесплатной квоты сохраняются;
их раздел `/premium-search` остаётся доступным по прежнему флагу.

В общей форме задаются запрос (3–160 символов), источники, трек Go, период
1–30 дней и до 100 результатов. Доступны предпросмотр, автоматическое сохранение
и сохранение с публикацией; действия с записью требуют подтверждения администратора.
Для ручного отбора оставьте **«Предпросмотр»**: он не записывает вакансии в Sheets
и не отправляет карточки. Неуверенные результаты можно включить отдельно. Карточки
показывают источник, ссылку **«Открыть оригинал»**, статус и запросы, по которым
найден пост. Threads нельзя добавить как Telegram-канал.

Статусы источников независимы: ошибка, квота или задержка одного не удаляет выдачу
остальных. Панель получает частичные результаты каждые две секунды во время поиска.
История и отмена сохраняются; **«Обновить поиск»** создаёт новый запуск с прежними
параметрами. Сохранение и публикация используют существующие два листа Sheets,
JSONL dedupe и Candidate publisher. Стабильный ID Threads — `threads:<id>`;
Telegram ID и ссылки не меняются. Новый основной реестр вакансий не вводится:
...
1. Создайте приложение с Threads use case в Meta for Developers.
2. Авторизуйте Threads-пользователя и запросите `threads_basic` и
   `threads_keyword_search`. Для поиска публичных чужих постов нужен одобренный
   доступ к `threads_keyword_search` через App Review; без него поиск ограничен
...
4. При необходимости обменяйте краткосрочный токен на долгосрочный. Срок такого
   ...
   действителен. Получение и обновление выполняет оператор: OAuth server и
   автоматическое обновление токенов в проекте не реализованы.

Требования и параметры сверены с официальными
[Threads keyword search](https://developers.facebook.com/docs/threads/keyword-search)
и [long-lived tokens](https://developers.facebook.com/docs/threads/get-started/long-lived-tokens/)
15 сентября 2026 года. Keyword search поддерживает `RECENT`, до 100 элементов
на страницу, `since`/`until`, `author_username` и `media_type`; географического и
языкового фильтра нет, поле `owner` для этой выдачи недоступно. Реализация запрашивает
`RECENT`, границы времени и курсоры; язык ru/en/und определяется локально.
Лимит Meta — до 2200 keyword-search запросов за 24 часа; локальный бюджет по
умолчанию ниже и не заменяет ограничения самого API.

### Настройки и фоновый сбор Threads

Все значения читаются из окружения при старте; примеры находятся в `.env.example`.

| Переменная | По умолчанию | Назначение и диапазон |
|---|---|---|
| `THREADS_ENABLED` | `false` | Включить адаптер Threads |
| `THREADS_ACCESS_TOKEN` | пусто | Пользовательский токен с разрешениями; без него источник недоступен |
| `THREADS_SEARCH_LIMIT` | `100` | До 1–1000 уникальных постов на запуск по всем вариантам запроса |
| `THREADS_QUERY_VARIANTS_LIMIT` | `6` | 2–12 вариантов запроса, включая русские и английские признаки найма |
| `THREADS_MAX_PAGES_PER_QUERY` | `2` | 1–10 страниц на вариант |
| `THREADS_TIMEOUT_SECONDS` | `20` | 1–120 секунд на HTTP-запрос |
| `THREADS_MAX_ATTEMPTS` | `3` | 1–5 попыток при временных сетевых ошибках/5xx |
| `THREADS_COLLECTION_INTERVAL_HOURS` | `6` | 0–168 часов; `0` отключает фоновый сбор |
| `THREADS_MAX_REQUESTS_PER_DAY` | `1000` | Общий локальный бюджет 1–2200 запросов в сутки, включая повторы |

Фоновый поиск Go-вакансий исполняется каждые шесть часов внутри уже существующего
live-процесса. Отдельный сервис, cron и новая Telegram session не нужны. Фоновые
запуски создают только предпросмотр; checkpoint последнего успешного сбора и
перекрытие окна позволяют повторно проверить свежие посты. Пока live-процесс не
работает, новые задания ждут исполнения.

Для Threads запрос расширяется вариантами Go/Golang и признаками найма на русском
и английском. Дешёвые правила отличают вакансии от резюме, обсуждений и рекламы;
Mistral вызывается для неоднозначного текста. Неуверенные решения остаются на
проверку. Повторно найденный ID хранится с несколькими поисковыми запросами.
При 429 соблюдается задержка; исчерпание локального бюджета откладывает запросы,
ошибка токена/прав отключает сетевые попытки до исправления и перезапуска.
Скрапинга, OCR картинок, обхода ATS и чтения сайтов по ссылкам из поста нет.
Полнота публичной выдачи зависит от доступа приложения и API Meta.

### API и проверка общего поиска

Защищённые маршруты: `GET /api/v1/search/capabilities`, `GET|POST .../runs`,
`GET .../runs/{id}`, `GET .../runs/{id}/results?offset=0&limit=25&include_review=true`,
`POST .../runs/{id}/cancel`, `POST .../results/{id}/actions`.
Мутации требуют admin session, CSRF и `confirmed=true`; действия результата —
`save`, `publish`, `reject`, `mark_duplicate`. API возвращает безопасные причины
ошибок и отдельные состояния источников.
...
Threads HTTP, классификация, лимиты и сбои проверяются моками; обычные тесты не
требуют токенов. Для ручного smoke используйте отдельное тестовое окружение:
настройте одобренный токен, запустите live и admin, выберите только Threads,
режим **«Предпросмотр»**, запрос `Golang developer`, небольшой лимит и период.
Проверьте ссылку на оригинал, русский/английский текст, независимые статусы,
отмену и историю; затем выполните `/search` разрешённым пользователем тестового
Candidate Bot. Реальный публичный smoke без credentials не выполнен; успешные
моки не подтверждают доступ Meta-приложения к чужим постам.
